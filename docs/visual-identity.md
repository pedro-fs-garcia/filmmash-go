...
- **Hover and focus should be twins**: pair every `:hover` rule with `:focus-within` (cards) or `:focus-visible` (controls). The reference implementation does this on duel cards (`:hover, :focus-within`), the search input (`:focus`), and the nav toggle (`:focus-visible` ring: `outline: 2px solid var(--gold-bright); outline-offset: 2px`). Film-list items and nav links currently lack focus styles — that's a known gap, not a pattern to copy.
